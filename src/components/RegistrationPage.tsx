...
/* Dedicated Standalone Print Component (Pure DOM Separation) */
export const PrintablePass: React.FC<PrintablePassProps> = ({
  fullName,
  college,
  eventNames,
  passId,
  ...
  return (
    <div className="pass-print-only">
      <div className="printable-card-body">
        {/* Top Header */}
        <div className="printable-header">
          <img src={srishtiLogo} alt="Srishti Logo" className="printable-logo" />
          <h1 className="printable-title">SRISHTI 2.7</h1>
          <p className="printable-subtitle">ST. THOMAS COLLEGE (AUTONOMOUS)</p>
          <div className="printable-badge">OFFICIAL DELEGATE PASS</div>
        </div>

        {/* Attendee Info */}
        <div className="printable-attendee">
          <span className="printable-label font-mono">ATTENDEE DELEGATE</span>
          <h2 className="printable-name">{fullName}</h2>
          <p className="printable-college">{college}</p>
        </div>

        {/* Registered Events */}
        <div className="printable-events">
          <span className="printable-label font-mono">REGISTERED EVENTS ({eventNames.length}):</span>
          <div className="printable-events-list">
            {eventNames.map((name, i) => (
              <div key={i} className="printable-event-item">
                <span className="printable-bullet">•</span>
                <span>{name}</span>
              </div>
            ))}
          </div>
        ...
        {/* Footer & Scannable QR Code */}
        <div className="printable-footer">
          <div className="printable-qr-wrap">
            <QrCode className="printable-qr" />
          </div>
          <div className="printable-serial">
            <span className="printable-id font-mono">SERIAL PASS ID: <strong>{passId}</strong></span>
            <span className="printable-utr font-mono">UTR: {paymentUtr}</span>
          </div>
        </div>
      </div>
    </div>
...
export const RegistrationPage: React.FC<RegistrationPageProps> = ({
  ...
  return (
    <>
      {/* =============================================
          1. INTERACTIVE SCREEN-ONLY WEB APPLICATION
          ============================================= */}
      <div className="screen-only-content min-h-screen bg-[#050608] text-[#E8E8EC] antialiased select-none relative overflow-x-hidden pt-20 pb-24">
        ...
        {!submittedRecord ? (
          ...
          /* Submission Confirmation — Minimalist, Cool Festival Pass */
          <main className="max-w-2xl mx-auto px-4 py-16 text-center space-y-8 relative z-10">
            ...
            {/* MINIMALIST & COOL VERTICAL FESTIVAL PASS CARD (INTERACTIVE PREVIEW) */}
            <div
      ...
      {/* =============================================
          2. STANDALONE DEDICATED PRINT PASS COMPONENT
          ============================================= */}
      {submittedRecord && (
        <PrintablePass
          fullName={submittedRecord.fullName}
          college={submittedRecord.college}
          eventNames={submittedRecord.selectedEventNames}
          passId={submittedRecord.passId}
          paymentUtr={submittedRecord.paymentUtr}
        />
      )}
    </>
  );
